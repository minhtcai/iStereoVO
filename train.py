...
class TrainVOStereoFlow(TorchFlow.TorchFlow):
    def __init__(self, workingDir, args, prefix = "", suffix = "", plotterType = 'Visdom'):
        ...
        self.transform =  StereoFlowVODataTransform((120, 160), resize_factor=4.0, flow_norm_factor=0.05, inverse_depth_factor=4.0, uncertainty=False)  
    
        if not args.test:
            self.trainDataloader = DataLoader(train_dataset, batch_size=64, shuffle=True)
            self.trainDataiter = iter(self.trainDataloader)
        self.testDataloader = DataLoader(test_dataset, batch_size=64, shuffle=False)
    ...
    def test(self):
        ...
        sample, transform = self.testDataloader.load_sample()
        
        self.voflownet.eval()

        with torch.no_grad():
            loss, trans_loss, rot_loss, motion = self.forward(sample, transform)
