...
from argumentssf import get_args
...
class TrainStereoFlow(TorchFlow.TorchFlow):
    def __init__(self, workingDir, args, prefix = "", suffix = "", plotterType = 'Visdom'):
        ...
        self.stereoflownet = StereoNet()

        if self.args.load_stereo_model:
            modelname = self.args.working_dir + '/models/' + self.args.stereo_model
            print(modelname)
        ...
        stereo_test_dataset = EurocDataset(base_path, trajectories=trajectories, load_depth=True, load_disparity=False, load_flow=False, 
                                load_left_img=True, load_right_img=True, load_next_img=False, left_camera=True)
        
        # stereo_train_dataset = CustomDataset(base_path, envs=environments, load_depth=True, load_flow=False,  load_left_img=True, load_right_img=True, load_next_img=False, left_camera=True)  
        # stereo_test_dataset = CustomDataset(base_path, envs=test_environments, load_depth=True, load_flow=False,  load_left_img=True, load_right_img=True, load_next_img=False, left_camera=True)  
        ...
        # flow_train_dataset = CustomDataset(base_path, envs=environments, load_depth=False, load_flow=True,  load_left_img=True, load_right_img=False, load_next_img=True, left_camera=False)  
        # flow_test_dataset = CustomDataset(base_path, envs=test_environments, load_depth=False, load_flow=True,  load_left_img=True, load_right_img=False, load_next_img=True, left_camera=False)  
        # self.flowtransform =  FlowDataTransform((480, 640), data_augment=True, resize_factor=2.0, rand_hsv=0.2, flow_norm_factor=0.05)
        ...
        self.stereo_testDataloader = DataLoader(stereo_test_dataset, batch_size=1, shuffle=False, num_workers=8)
        self.stestDataiter = iter(self.stereo_testDataloader)
        # self.flow_testDataloader = DataLoader(flow_test_dataset, batch_size=64, shuffle=True, num_workers=8)
    ...
    def forward_stereo(self, sample, transform, mask=False): 
        ...
        targetdisp = 47.9/(targetdepth + 10e-6) #80 for tartan ; 47.9 for euroc
    ...
    def forward_flow(self, sample, transform, use_mask=False): 
        ...
        # print(inputTensor1.shape)
        inputTensor2 = sample['next_img']
        inputTensor = torch.cat(inputTensor1, inputTensor2)
        inputTensor = inputTensor.cuda()
        targetflow = sample['flow'].squeeze(1).cuda()

        with torch.no_grad():
            inputTensor, targetflow = transform(inputTensor, targetflow)
        img0Tensor = inputTensor[:,0,:,:,:]
        img1Tensor = inputTensor[:,1,:,:,:]
        _, _, output, output_unc  = self.stereoflownet(img0=img0Tensor, img0n=img1Tensor, stereo=False, flow=True)
        ...
        return loss/self.flow_normfactor, loss_nounc, (output, output_unc), (img0Tensor, img1Tensor, targetflow)
    ...
    def train(self):
        ...
        # loadtime2 = time.time()
        # self.stereoOptimizer.zero_grad()
        # loss_flow, loss_flow_nounc, _, _ = self.forward_flow(sample, self.flowtransform)
        # loss_flow.backward()
        # self.stereoOptimizer.step()
        # nntime2 = time.time()
        ...
        if self.count % self.args.print_interval == 0:
            ...
            self.logger.info("%s #%d - %s lr: %.6f - time load/bp (%.2f / %.2f)"  % (self.args.exp_prefix[:-1], 
                self.count, losslogstr, self.lr, loadtime1-starttime1, nntime1-loadtime1))
        ...
        wandb.log({"train_loss":loss_stereo.item(), "train_loss_nounc": loss_stereo_nounc.item()})
    ...
    def visoutput_stereo(self, output, leftimg, rightimg, targetdisp, save_surfix='', unc=None):
        ...
        img1 = np.concatenate((leftimg, dispvis),axis=0)
        img2 = np.concatenate((rightimg, resvis),axis=0)
    ...
    def visoutput_flow(self, output, img0Tensor, img1Tensor, targetflow, save_surfix='', output_unc=None):
        ...
        disp1 = np.concatenate((img1, diffvis),axis=0)
    ...
    def test(self, save_img=False, save_surfix_stereo='', save_surfix_flow=''):
        ...
        return (stereo_loss_nounc, stereo_err3)
    ...
    def test_stereo(self, save_img=False, save_surfix=''):
        ...
        # wandb.log({"test_loss":lossnum, "test_loss_nounc": loss_nounc})
    ...
    def test_flow(self, save_img=False, save_surfix=''):
        try:
            ...
            sample = next(self.ftestDataiter)
        with torch.no_grad():
            loss, loss_flow, (output, output_unc), (img0Tensor, img1Tensor, targetflow) = self.forward_flow(sample, self.flowtransform)
        ...
        if save_img:
            self.visoutput_flow(output, img0Tensor, img1Tensor, targetflow, save_surfix, output_unc)

        finish = self.test_count*output.shape[0]>= self.flow_testDataloader.datalens[0] # only test one epoch when --test
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not args.test:
            run=wandb.init(project=f"stereoonly", config=CONFIG)
            while True:
            ...
            while True:
                ...
                (stereoloss, stereo_err3) = trainStereoFlow.test(save_img = saveimg)
